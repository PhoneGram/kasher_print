...
// تحديث الأرصدة من فاتورة مدفوعة
function updateBalancesFromInvoice(invoice, data) {
    ...
    const printQty = parseInt(invoice.printQty) || 0;
    
    const totalAmount = printPrice * printQty;
    const mainAmount = Math.floor(totalAmount * mainPercentage / 100);
    const workerAmount = totalAmount - mainAmount;
    
    data.balances.main = (data.balances.main || 0) + mainAmount;
    data.balances.worker = (data.balances.worker || 0) + workerAmount;
    
    // لا نضيف حركة في المحفظة (حسب المتطلبات: المحفظة للإيداع والسحب اليدوي فقط)
}
...
function getDayStatistics(date) {
    ...
    const printPrice = settings.printPrice || 1000;
    const workerPercentage = 100 - (settings.mainPercentage || 75);
    ...
    const invoiceDetails = dayInvoices.map((inv, index) => {
        ...
        const amount = printPrice * qty;
        const workerFee = Math.floor(amount * workerPercentage / 100);
    ...
    return {
        ...
        totals: {
            prints: totalPrints,
            income: totalIncome,
            workerFees: Math.floor(totalIncome * workerPercentage / 100)
        }
    };
}
...
function getMonthStatistics(yearMonth) {
    ...
    monthInvoices.forEach(inv => {
        ...
        daysMap[day].workerFees += Math.floor(amount * workerPercentage / 100);
    });
...
function getYearStatistics(year) {
    ...
    yearInvoices.forEach(inv => {
        ...
        monthsMap[month].workerFees += Math.floor(amount * workerPercentage / 100);
    });
